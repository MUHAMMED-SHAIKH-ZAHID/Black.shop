...
//   })()
  ...
  const wishlist = async (id) => {
    try {
        const res = await axios.get(`/wishlist/${id}`).then((e) => {
            if (e.data.response.wishlistDuplicate) {
              swal({
              ...
              // swal("", "Added to cart", "/images/wishlist.jpg")
            }
             else {
                console.log("its in else of wishlist axios and added to cart")
            }
        })
    } catch (err) {
        console.error(err)
...
class Slider {
  constructor() {
    ...
    this.images = [
    "/images/courossel/bg1.jpg",
    "/images/courossel/bg2.jpg",
    "/images/courossel/bg3.jpg"];
  ...
  loadTextures() {
    ...
    "/images/courossel/rock-_disp.png",
    this.render);
